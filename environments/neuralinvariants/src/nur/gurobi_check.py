...
def bMul(bvar, number, bw_obj, bvsizeB_larger, dpt):
    ...
    t1 = tm.mk_term(
        bw.Kind.BV_SHL, [bvar, tm.mk_bv_value(bvsizeB_larger, pow_2)]
    )  # tm.mk_term(bw.Kind.BV_MUL, [bvar, tm.mk_bv_value(bvsizeB, 2**pow_2)])#tm.mk_term(bw.Kind.BV_SHL, [bvar, tm.mk_bv_value(bvsizeB, pow_2)])
    if bias > 2:
        t2 = bMul(
            bvar, bias, bw_obj, bvsizeB_larger, dpt + 1
        )  # tm.mk_term(bw.Kind.BV_MUL, [bvar, tm.mk_bv_value(bvsizeB, bias)])
    else:
    ...
    if sign == 1:
        ...
        return tm.mk_term(
            bw.Kind.BV_MUL,
            [tm.mk_term(bw.Kind.BV_ADD, [t1, t2]), tm.mk_bv_value(bvsizeB_larger, -1)],
        )
...
def bDotnew(arrVar, arrNum, bw_obj, F_prec, bits):
    ...
    if ln == 1:
        ...
        tmp = tm.mk_term(
            bw.Kind.BV_ASHR,
            [bMul(var, arrNum[0], bw_obj, bvsizeB_larger, 0), F_btor_larger],
        )
    ...
    return tm.mk_term(
        bw.Kind.BV_ADD,
        [
            bDotnew(arrVar[:part], arrNum[:part], bw_obj, F_prec, bits),
            bDotnew(arrVar[part:], arrNum[part:], bw_obj, F_prec, bits),
        ],
    )
...
def bElemMul(bw_obj, mat1, scaled_weight, F_prec, bits, isDebug=False):
    ...
    for i in range(len(mat1)):
        ...
        tmp = tm.mk_term(
            bw.Kind.BV_ASHR,
            [bMul(var, scaled_weight[i], bw_obj, bvsizeB_larger, 0), F_btor_larger],
        )
        tmp = tm.mk_term(bw.Kind.BV_EXTRACT, [tmp], [bits - 1, 0])
        out.append(tmp)
    return out
...
def bSum(arrVar, bw_obj, F_prec, bits):
    ...
    return tm.mk_term(
        bw.Kind.BV_ADD,
        [
            bSum(arrVar[:part], bw_obj, F_prec, bits),
            bSum(arrVar[part:], bw_obj, F_prec, bits),
        ],
    )
...
def bSignFnc(bw_obj, arrVar, bits, isLast, F_prec, gap):
    ...
    for i in range(len(arrVar)):
        ...
        is_zero_v2 = tm.mk_term(
            bw.Kind.AND,
            [
                tm.mk_term(bw.Kind.BV_SGT, [bgap, arrVar[i]]),
                tm.mk_term(bw.Kind.BV_SGT, [arrVar[i], bgap_neg]),
            ],
        )
        ...
        sign = tm.mk_term(
            bw.Kind.ITE,
            [is_zero_v1, bZero, tm.mk_term(bw.Kind.ITE, [msb, bneg1, bpos1])],
        )
        signs.append(sign)
...
def BLessThanEps(rank_before, rank_after, delta, context, F_prec, isDebug=False):
    ...
    res = tm.mk_term(
        bw.Kind.BV_SLT, [tm.mk_term(bw.Kind.BV_SUB, [rank_before, dt]), rank_after]
    )
    if isDebug:
        breakpoint()
    return res
    ...
    for i in range(len(l1)):
        res.append(
            tm.mk_term(
                ...
                [
                    tm.mk_term(
                        bw.Kind.EQUAL, [l1[i], tm.mk_bv_value(bvsizeB, int(l2[i]))]
                    )
                ],
            )
        )
...
def bCAV_NRF(bw_obj, nnparam, clparam, inp, scale, F_prec, bits, gap, isDebug=False):
    ...
    # If(wi > 0, zi, 0)
    V_arr = [
        tm.mk_term(bw.Kind.ITE, [tm.mk_term(bw.Kind.BV_SGT, [wi, bZero]), zi, bZero])
    ...
    for cex_i in range(1):
        ...
        for cex in cex_trans:
            parser.bitwuzla().assert_formula(
                bAnd(bSetListUnEqual(curr_vars, cex[1], bw_obj), bw_obj)
            )
            parser.bitwuzla().assert_formula(
                bAnd(bSetListUnEqual(next_vars, cex[3], bw_obj), bw_obj)
            )
        ...
        if res == bw.Result.SAT:
            ...
            print(
                f"{Fore.WHITE}\tBitwuzla Rank [{todecimal(V_cur, bw_obj, bits)/2**F_prec} -> {todecimal(V_nex, bw_obj, bits)/2**F_prec}]; Numpy Rank [{V_eval} -> {V_nex_eval}]; ; Numpy RankQ [{V_eval_q/2**F_prec} -> {V_nex_eval_q/2**F_prec}]  {Style.RESET_ALL}"
            )
...
def check_init(
    ...
    for q0 in q_set:
        ...
        if res == bw.Result.SAT:
            ...
            print(
                f"{Fore.WHITE}\tBitwuzla Rank [{todecimal(V_cur, bw_obj, bits)/2**F_prec}]; Numpy Rank [{V_eval}]; ; Numpy RankQ [{V_eval_q/2**F_prec}]  {Style.RESET_ALL}"
            )
        else:
