...
NNParams = list[Optional[list[tuple[np.ndarray, np.ndarray]]]]
LinParams = list[tuple[np.ndarray, np.ndarray]]
Samples = list[tuple[int, np.ndarray, int, np.ndarray]]
InitSamples = list[tuple[int, np.ndarray]]


def add_lin_param(m, n_in, n_out, mat_name, vec_name, **kwargs):
...
def add_nn_param(m, sizes, W_prfx="W", b_prfx="b", psfx="", **kwargs):
    """Construct a list of `(W,b)` parameter pairs for each NN layer, according to `sizes`."""
    return [
        add_lin_param(
            m, n_in, n_out, W_prfx + str(i) + psfx, b_prfx + str(i) + psfx, **kwargs
        )
        for i, (n_in, n_out) in enumerate(zip(sizes[:-1], sizes[1:]))
    ]


def add_lin_sing_layer(m, W, b, x, gap, ptype, psfx=""):
...
def add_lin_sign_nn(m, param, x, M, gap, ptype, psfx=""):
    ...
    for i, (W, b) in enumerate(param[1:], 1):
        y = add_lin_sing_layer(m, W, b, x=y, gap=gap, ptype=ptype, psfx=str(i) + psfx)

    return y


def add_funky_nn(m, nnparam: Optional[NNParams], A, c, x, M, gap, ptype, psfx=""):
    """
    ...
    alpha = m.addMVar(
        shape=(A.shape[0]), vtype=ptype, name="alpha" + psfx, lb=alpha_lb, ub=alpha_ub
    )  # just a guess - these bounds are to be checked
    if nnparam == None:
        m.addConstr((A @ x + c == alpha))
        return sum(alpha)

    y = add_lin_sign_nn(m, nnparam, x, M, gap=gap, ptype=ptype, psfx=psfx)
    m.update()
...
class MIPLearn:
    ...
    def __init__(self, accept, size, P, M, ptype=GRB.INTEGER):
        ...
        if len(size) == 1:
            ...
            self.nnparam = [
                add_nn_param(self.m, size, vtype=ptype, lb=-P, ub=P, psfx="q" + str(i))
    ...
    def get_V(self, q: int, x: np.ndarray):
        """Create the MIP expression for the ranking function `V(q, x)` using `addFunkyNN`."""
        key = (q, x.tobytes())
    ...
    def add_samples(self, samples: Samples):
        """
        Add transition constraints (samples of`(q, state, q_next, next_state)`)
        enforcing ranking conditions.
        """
        if len(samples) == 0:
            return
        assert all(
            [
                all(np.mod(s0, 1) == 0) and all(np.mod(s1, 1) == 0)
                for _, s0, _, s1 in samples
            ]
        )
        ...
        for q, s, q_next, s_next in samples:
            ...
            V, V_next = self.get_V(q, s), self.get_V(q_next, s_next)
            if M_enc:
                self.m.addConstr((z - 1) * M_out <= V - (self.kappa + 1))
                self.m.addConstr(V - (self.kappa + 1) <= z * M_out)
                self.m.addConstr(
                    V_next - V + self.accept[q] <= z * (M_out + self.accept[q])
                )
            else:
    ...
    def add_init_invar(self, samples: InitSamples):
        """Add initial state invariants"""
        if len(samples) == 0:
            return
    ...
    def guess(self) -> tuple[Optional[NNParams], Optional[LinParams], Optional[Number]]:
        """
        Solve the MIP, return extracted `(nnparam, linparam, kappa)`
        ...
        if self.nnparam[0] is None:
            assert all(paramq is None for paramq in self.nnparam)
            nnparam_X = self.nnparam
        else:
            assert all(paramq is not None for paramq in self.nnparam)
...
def quant(a, F):
    """
    Recursively quantize arrays/tuples of numpy arrays by
...
def optimal_F(
    ...
    for q, s, q_next, s_next in samples:
        V = eval_funky_nn(nnparam[q], *linparam[q], s.astype(int))
        V_next = eval_funky_nn(nnparam[q_next], *linparam[q_next], s_next.astype(int))
        # print(f"{Fore.RED} V(q={q},{s}) = {V} -> V(q={q_next},{s_next}) = {V_next} : {Style.RESET_ALL}")
        if (V < kappa) and (V < V_next + accept[q]):
            print(
                f"{Fore.RED} V(q={q},{s}) = {V} -> V(q={q_next},{s_next}) = {V_next} : {Style.RESET_ALL}"
            )
            breakpoint()
    ...
    for F in range(minimum, maximum):
        ...
        if_success = True
        for q, s, q_next, s_next in samples:
            V = eval_funky_nn(qnnparam[q], *qlinparam[q], s.astype(int))
            V_next = eval_funky_nn(
                qnnparam[q_next], *qlinparam[q_next], s_next.astype(int)
            )
    ...
    raise RuntimeError(f"Could not find an optimal F between {minimum} and {maximum}")


def eval_funky_nn(param, A, c, x):
    """
    Debug Function
    Evaluate the unquantized (floating-point)
...
def eval_quant_nn(param, A, c, x, scale, gap, F_prec, printing: bool = False):
    """
    Debug Function
    ...
    return np.dot(z1_quant, np.maximum(y, 0.0))
...
def test_gurobi_nn(
    ...
    linparam: LinParams,
    samples: Samples,
    init_samples: InitSamples,
    accept: list[int],
    printing: bool = False,
):
    """
    Test the found NN with numpy to see if it indeed satisfies all constraints.
    Without proper hyperparameters, it appears this can fail sometimes.
    """
    never_failed = True
    ...
    for q, s, q_next, s_next in samples:
        V = eval_funky_nn(
            quant(nnparam[q], best_F), *(quant(linparam[q], best_F)), s.astype(int)
        )
        V_next = eval_funky_nn(
            quant(nnparam[q_next], best_F),
            *(quant(linparam[q_next], best_F)),
            s_next.astype(int),
        )
        ...
        if printing:
            print(
                f"{Fore.YELLOW} V(q={q},{s}) = {V} -> V(q={q_next},{s_next}) = {V_next} : {res} {Style.RESET_ALL}"
            )

    if printing:
        print("-------[Init Invar]-------")
    for q, s in init_samples:
        V = eval_funky_nn(
            quant(nnparam[q], best_F), *(quant(linparam[q], best_F)), s.astype(int)
        )
    ...
    if not never_failed:
        print(
            f"{Fore.RED} [Exiting] Failed at a sample; Tune hyperparameter and retry! {Style.RESET_ALL}"
        )
        breakpoint()
...
def gurobi_nn_train(
    ...
    mip_learn: MIPLearn,
    new_samples: Samples,
    new_inits: InitSamples,
    samples: Samples,
    init_samp: InitSamples,
    scale,
    P,
    accept: list[int],
    size,
    ...
    mip_learn.add_samples(new_samples)
    mip_learn.add_init_invar(new_inits)
    nnparam, linparam, kappa = mip_learn.guess()

    if nnparam is None or linparam is None or kappa is None:
        assert nnparam is None and linparam is None and kappa is None
        return None, None, None, None

    # Copious amounts of asserts just for me to figure out how this codebase works.
    if nnparam is not None:
        assert isinstance(nnparam, list)
        if not all(paramq is None for paramq in nnparam):
            assert all(isinstance(paramq, list) for paramq in nnparam)
            assert all(
                isinstance(w, np.ndarray) and isinstance(b, np.ndarray)
                for paramq in nnparam
                for w, b in paramq
            )
    if kappa is not None:
        assert isinstance(kappa, Number)

    assert isinstance(accept, list)
    assert all(a == 0 or a == 1 for a in accept)

    best_F = optimal_F(samples, accept, nnparam, linparam, kappa)
    assert isinstance(best_F, int)

    test_gurobi_nn(best_F, kappa, nnparam, linparam, samples, init_samp, accept)
