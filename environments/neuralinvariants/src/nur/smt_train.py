...
def add_lin_param(n_in, n_out, mat_name, vec_name, vtype="int", P=None):
    ...
    W = [
        [Symbol(f"{mat_name}_{i}_{j}", typ) for j in range(n_in)] for i in range(n_out)
    ]
...
def add_lin_sign_layer(W, b, x, gap, ptype, psfx="", P=None):
    ...
    C = [
        [
            Symbol(f"C{psfx}_{i}_{j}", REAL if ptype == "real" else INT)
            for j in range(n_in)
        ]
        for i in range(n_out)
    ]
    ...
    for i in range(n_out):
        for j in range(n_in):
            ...
            constraints.append(
                Implies(Equals(x[j], Int(0)), Equals(C[i][j], Minus(Int(0), W[i][j])))
            )
    ...
    for i in range(n_out):
        ...
        row_sum = Plus(C[i])
        constraints.append(
            Implies(Equals(y[i], Int(0)), LE(Plus(row_sum, b[i]), Int(0)))
        )
        constraints.append(
            Implies(Equals(y[i], Int(1)), GT(Plus(row_sum, b[i]), Int(0)))
        )
...
def add_lin_sign_nn(param, x, gap, ptype, psfx="", P=None):
    ...
    for i in range(n_out):
        ...
        constraints.append(
            Implies(Equals(y[i], Int(0)), LE(Plus(dot_expr, b0[i]), Int(0)))
        )
        constraints.append(
            Implies(Equals(y[i], Int(1)), GT(Plus(dot_expr, b0[i]), Int(0)))
        )

    # Subsequent layers: use the sign layer transformation
    for i, (W, b) in enumerate(param[1:], 1):
        y, cons_layer = add_lin_sign_layer(
            W, b, x=y, gap=gap, ptype=ptype, psfx=str(i) + psfx, P=P
        )
...
def add_funky_nn(nnparam, A, c, x, M, gap, ptype, psfx="", P=None):
    ...
    n_out = len(A)
    alpha = [
        Symbol(f"alpha{psfx}_{i}", REAL if ptype == "real" else INT)
        for i in range(n_out)
    ]
    for i in range(n_out):
        if P is not None:
    ...
    for i in range(n_out):
        ...
        constraints.append(
            Implies(Equals(y[i], Int(1)), Equals(Plus(dot_expr, c[i]), alpha[i]))
        )
...
class SMTLearn:
    ...
    def __init__(self, accept, size, P, M, ptype="int"):
        ...
        for i in range(len(accept)):
            ...
            if len(size) != 1:
                nn_layers, cons_nn = add_nn_param(
                    size, W_prefix=f"W", b_prefix=f"b", psfx=f"_q{i}", vtype=ptype, P=P
                )
        ...
        if P is not None:
            self.constraints.append(
                And(GE(self.kappa, Int(-P)), LE(self.kappa, Int(P)))
            )

        self.cache = {}  # To avoid rebuilding parts of the network
        self.debug_cache = (
            []
        )  # For logging which constraints are associated with which sample
    ...
    def add_samples(self, samples):
        ...
        for q, s, q_next, s_next in samples:
            ...
            self.constraints.append(
                Implies(Equals(z, Int(0)), LE(V_next, Plus(V, Int(-self.accept[q]))))
            )
            # (z == 1) implies (V >= kappa + 1)
            self.constraints.append(
                Implies(Equals(z, Int(1)), GE(V, Plus(self.kappa, Int(1))))
            )
        ...
        if self.nnparam[0] == None:
            ...
            for network_layers in self.nnparam:
                ...
                for W_matrix, b_vector in network_layers:
                    ...
                    for row in W_matrix:
                        row_vals = [
                            to_python_value(model.get_value(sym)) for sym in row
                        ]
...
def smtNNtrain(
    ...
    for (
        ...
        samples
    ):  # + [(0, np.array([float(800)]), 1, np.array([float(0)])), (1, np.array([float(800)]), 1, np.array([float(0)]))]:
        V = evalFunkyNN(
