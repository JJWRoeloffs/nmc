"""
Run nuXmv from an imported config file. This needs to have the following things defined:

start_ex: int
    The index of the first file to run. Used to skip files
N_lims: list[int]
    For nuXmv, all that matters is that this list contains an equal amount of unique numbers,
...
nuXmvSpec: str | None
    The nuXmvSpec to check, or none if there is a SVSpec instead.
...
parser = argparse.ArgumentParser(prog="nuXmv entrypoint")
...
resultspath = arguments.resultspath / f"{executable}_{taskname}"
resultspath.mkdir()
...
for dut_i in range(argsmodule.start_ex, len(argsmodule.N_lims)):
    ...
    tempdir = Path() / f"generated_{taskname}_{executable}_{dut_i}"
    tempdir.mkdir()
    ...
    smvcontent = targetpath.read_text()
    smvcontent = smvcontent[smvcontent.find("-- Generated by EBMC") :]
    if argsmodule.SVSpec is None:
        smvcontent += "\n\n" + argsmodule.nuXmvSpec
    targetpath.write_text(smvcontent)

    # Actually run the thing
    start = perf_counter()
    subprocess.run([executable, targetpath], check=True)
    duration = perf_counter() - start

    print(f"Ran nuXmv on {arguments.runargs_path}, IC3={arguments.ic3}, ID {idtxt}")
