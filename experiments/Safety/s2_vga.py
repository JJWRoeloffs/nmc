...
specTXT = "X G ((disp_ena & !rst) -> ((v_sync <-> X !v_sync) | (!v_sync <-> X v_sync)))"
module_name = "VGA"
...
file_name = "vga"
LTLSpec = "LTLSPEC X G ((Verilog.VGA.disp_ena = TRUE & Verilog.VGA.rst = FALSE) -> ((Verilog.VGA.v_sync = TRUE <-> X Verilog.VGA.v_sync = FALSE) | (Verilog.VGA.v_sync = FALSE <-> X Verilog.VGA.v_sync = TRUE)))"
SVSpec = (
...
try:
    ...
    def spec_automata(ctx, q_cur, curr_vars, V0, q_nex, next_vars, V1, non_state, s):
        ...
        if q_cur == 0 and q_nex == 1:
            ...
            cases.append(
                [
                    nur.Bset(non_state, "disp_ena", 1, ctx),
                    nur.BUnSet(curr_vars, "v_sync", 1, ctx),
            ...
            cases.append(
                [
                    nur.Bset(non_state, "disp_ena", 1, ctx),
                    nur.Bset(curr_vars, "v_sync", 1, ctx),
                    nur.BUnSet(non_state, "rst", 1, ctx),
                ]
            )
            ...
            cases.append(
                [
                    nur.BUnSet(non_state, "disp_ena", 1, ctx),
                    nur.Bset(curr_vars, "v_sync", 1, ctx),
                ]
            )
            cases.append(
            ...
            cases.append([nur.BUnSet(curr_vars, "v_sync", 1, ctx)])
            ...
            cases.append([nur.Bset(curr_vars, "v_sync", 1, ctx)])
