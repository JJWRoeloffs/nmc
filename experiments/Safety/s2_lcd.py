...
try:
    ...
    def spec_automata(ctx, q_cur, curr_vars, V0, q_nex, next_vars, V1, non_state, s):
        ...
        if q_cur == 0 and q_nex == 1:
            ...
            cases.append([b_set(non_state, "busy", 1, ctx)])
        elif q_cur == 1 and q_nex == 2:
            cases.append([b_unset(non_state, "busy", 1, ctx)])

        elif q_cur == 2 and q_nex == 2:
            cases.append(
                [
                    b_unset(non_state, "busy", 1, ctx),
                    b_unset(curr_vars, "state", 1, ctx),
                ]
            )
        elif q_cur == 2 and q_nex == 1:
            cases.append(
                [
                    b_set(non_state, "busy", 1, ctx),
                    b_unset(curr_vars, "state", 1, ctx),
                ]
            )
        elif q_cur == 2 and q_nex == 3:
            cases.append([b_set(curr_vars, "state", 1, ctx)])
